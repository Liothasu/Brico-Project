...
class CommentController extends AbstractController
{
    ...
    public function addComment(Request $request): Response
    {
        ...
        $blog = $this->blogRepository->findOneBy(['id' => $data['blog']]);

        $comment = $this->commentService->add($data, $blog);
        ...
        $html = $this->renderView('pages/comment/index.html.twig', [
            'comment' => $comment
        ]);
        ...
        return $this->json([
            ...
            'detail' => [
                ...
                'numberOfComments' => $this->commentRepo->count(['blog' => $blog])
            ],
            'message' => $html,
        ]);
    }
    ...
    public function addAnswer(Request $request): Response
    {
        ...
        $answer = $this->commentService->add($data, $comment->getBlog(), $comment, true);
    ...
    public function editComment(?Comment $comment, Request $request): Response
    {
        ...
        if ($request->isMethod(Request::METHOD_GET)) {
            $cardText = $this->renderView('pages/comment/_card_text.html.twig', [
                'action' => 'edit',
                'content' => $comment->getContent()
            ]);

            $cardFooter = $this->renderView('pages/comment/_card_footer.html.twig', [
                'action' => 'edit',
                'id' => $comment->getId()
            ]);
    ...
    public function deleteComment(?Comment $comment): Response
    {
        ...
        return $this->json([
            ...
            'detail' => [
                'numberOfComments' => $this->commentRepo->count(['blog' => $comment->getBlog()])
            ],
            'message' => null
        ]);
    }
